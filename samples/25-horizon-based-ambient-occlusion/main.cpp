...
int main()
{
    ...
    sf::Window window(videoMode, "Hello, HBAO!", sf::Style::Default, settings);
    ...
    std::cout << "[INFO] Compiling HBAO vertex shader...";

    auto hbaoVertexSource = globjects::Shader::sourceFromFile("media/hbao.vert");
    auto hbaoVertexShaderTemplate = globjects::Shader::applyGlobalReplacements(hbaoVertexSource.get());
    auto hbaoVertexShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_VERTEX_SHADER), hbaoVertexShaderTemplate.get());

    if (!hbaoVertexShader->compile())
    {
        std::cerr << "[ERROR] Can not compile HBAO vertex shader" << std::endl;
        return 1;
    }

    std::cout << "[INFO] Compiling HBAO fragment shader...";

    auto hbaoFragmentSource = globjects::Shader::sourceFromFile("media/hbao.frag");
    auto hbaoFragmentShaderTemplate = globjects::Shader::applyGlobalReplacements(hbaoFragmentSource.get());
    auto hbaoFragmentShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_FRAGMENT_SHADER), hbaoFragmentShaderTemplate.get());

    if (!hbaoFragmentShader->compile())
    {
        std::cerr << "[ERROR] Can not compile HBAO fragment shader" << std::endl;
        return 1;
    }

    std::cout << "done" << std::endl;

    std::cout << "[DEBUG] Linking HBAO shaders..." << std::endl;

    auto hbaoProgram = std::make_unique<globjects::Program>();
    hbaoProgram->attach(hbaoVertexShader.get(), hbaoFragmentShader.get());

    std::cout << "done" << std::endl;

    std::cout << "[INFO] Compiling blur vertex shader...";
    ...
    std::cout << "[INFO] Preparing hbao noise...";
    ...
    auto hbaoNoiseTexture = std::make_unique<globjects::Texture>(static_cast<gl::GLenum>(GL_TEXTURE_2D));

    hbaoNoiseTexture->setParameter(static_cast<gl::GLenum>(GL_TEXTURE_MIN_FILTER), static_cast<GLint>(GL_NEAREST));
    hbaoNoiseTexture->setParameter(static_cast<gl::GLenum>(GL_TEXTURE_MAG_FILTER), static_cast<GLint>(GL_NEAREST));
    hbaoNoiseTexture->setParameter(static_cast<gl::GLenum>(GL_TEXTURE_WRAP_S), static_cast<GLint>(GL_REPEAT));
    hbaoNoiseTexture->setParameter(static_cast<gl::GLenum>(GL_TEXTURE_WRAP_T), static_cast<GLint>(GL_REPEAT));

    hbaoNoiseTexture->image2D(
        0,
        static_cast<gl::GLenum>(GL_RGB32F),
        glm::vec2(8.0f, 8.0f),
        0,
    ...
    while (window.isOpen())
    {
        ...
        // second render pass - calculate & blur the ambient occlusion
        {
            ...
            hbaoNoiseTexture->bindActive(5);

            hbaoProgram->setUniform("positionTexture", 2);
            hbaoProgram->setUniform("normalTexture", 3);
            hbaoProgram->setUniform("albedoTexture", 4);

            hbaoProgram->setUniform("hbaoNoiseTexture", 5);

            hbaoProgram->setUniform("cameraPosition", cameraPos);
            hbaoProgram->setUniform("projection", cameraProjection);
            hbaoProgram->setUniform("view", cameraView);
            ...
            hbaoNoiseTexture->unbindActive(5);
        ...
        // third render pass - merge textures from the deferred rendering pre-pass into a final frame
        {
            ...
            deferredRenderingFinalPassProgram->setUniform("hbaoTexture", 5);
