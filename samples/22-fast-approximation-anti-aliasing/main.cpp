...
int main()
{
    ...
    sf::Window window(videoMode, "Hello, FXAA!", sf::Style::Default, settings);
    ...
    auto fxaaVertexSource = globjects::Shader::sourceFromFile("media/fxaa.vert");
    auto fxaaVertexShaderTemplate = globjects::Shader::applyGlobalReplacements(fxaaVertexSource.get());
    auto fxaaVertexShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_VERTEX_SHADER), fxaaVertexShaderTemplate.get());

    if (!fxaaVertexShader->compile())
    {
    ...
    auto fxaaFragmentSource = globjects::Shader::sourceFromFile("media/fxaa.frag");
    auto fxaaFragmentShaderTemplate = globjects::Shader::applyGlobalReplacements(fxaaFragmentSource.get());
    auto fxaaFragmentShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_FRAGMENT_SHADER), fxaaFragmentShaderTemplate.get());

    if (!fxaaFragmentShader->compile())
    {
    ...
    auto fxaaProgram = std::make_unique<globjects::Program>();
    fxaaProgram->attach(fxaaVertexShader.get(), fxaaFragmentShader.get());
