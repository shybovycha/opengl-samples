...
int main()
{
    ...
    sf::Window window(videoMode, "Hello, MSAA!", sf::Style::Default, settings);
    ...
    std::cout << "[INFO] Compiling FXAA vertex shader...";

    auto msaaVertexSource = globjects::Shader::sourceFromFile("media/msaa.vert");
    auto msaaVertexShaderTemplate = globjects::Shader::applyGlobalReplacements(msaaVertexSource.get());
    auto msaaVertexShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_VERTEX_SHADER), msaaVertexShaderTemplate.get());

    if (!msaaVertexShader->compile())
    {
        std::cerr << "[ERROR] Can not compile FXAA vertex shader" << std::endl;
        return 1;
    }

    std::cout << "[INFO] Compiling FXAA fragment shader...";

    auto msaaFragmentSource = globjects::Shader::sourceFromFile("media/msaa.frag");
    auto msaaFragmentShaderTemplate = globjects::Shader::applyGlobalReplacements(msaaFragmentSource.get());
    auto msaaFragmentShader = std::make_unique<globjects::Shader>(static_cast<gl::GLenum>(GL_FRAGMENT_SHADER), msaaFragmentShaderTemplate.get());

    if (!msaaFragmentShader->compile())
    {
        std::cerr << "[ERROR] Can not compile FXAA fragment shader" << std::endl;
        return 1;
    }

    std::cout << "done" << std::endl;

    std::cout << "[DEBUG] Linking FXAA shaders..." << std::endl;

    auto msaaProgram = std::make_unique<globjects::Program>();
    msaaProgram->attach(msaaVertexShader.get(), msaaFragmentShader.get());
    ...
    while (window.isOpen())
    {
...

        {
            ...
            msaaProgram->setUniform("diffuseTexture", 0);
